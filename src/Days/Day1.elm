module Days.Day1 exposing (getPuzzleAnswer, getPuzzleAnswer2)
    ...
    let
        shiftedInputList =
            case List.head inputList of
                Just a ->
                    List.take (List.length inputList - 1) <|
                        (a :: inputList)
        ...
        updatedTupleList =
            List.map2
                (\shiftedItem item ->
                    ( getIntFromChar shiftedItem, getIntFromChar item )
                )
                shiftedInputList
                inputList
    in
...
getPuzzleAnswer2 =
    let
        firstHalfList =
            List.take (List.length inputList // 2) inputList

        secondHalfList =
            List.drop (List.length inputList // 2) inputList

        updatedTupleList =
            List.map2
                (\f s ->
                    ( getIntFromChar f, getIntFromChar s )
                )
                firstHalfList
                secondHalfList
    in
