module Days.Day4 exposing (getPuzzleAnswer, getPuzzleAnswer2)
...
isInvalidPassphrase : List String -> String -> Bool
isInvalidPassphrase words row =
    List.any
        (\word ->
            let
                wordOccurrences =
                    String.indexes word (toString row)
            in
            List.length wordOccurrences > 1
        )
        words
...
isAnagram firstWord secondWord =
    let
        ( firstWordLength, secondWordLength ) =
            ( String.length firstWord, String.length secondWord )
    in
    if firstWordLength == secondWordLength then
        String.all
            (\c ->
                let
                    firstWordIndexes =
                        String.indexes (String.fromChar c) firstWord

                    secondWordIndexes =
                        String.indexes (String.fromChar c) secondWord
                in
                if List.length firstWordIndexes == List.length secondWordIndexes then
                    True
                else
                    False
            )
